...
(`<owner>/<repo>` 형식으로 GitHub repo 등록. 본인 namespace fallback이면 `GGGGGANG/account-partition`.)
...
v0.4부터 "계정 연동 해제"는 완전 자동화: `auth logout` + zshrc 블록 제거 + config dir 아카이브.
...
### Keychain 작업 가드 (리비전 2/3 추가, P0-3·P1-1 대응)

**리비전 3 결정**: §14 검증 2 결과 keychain hash 알고리즘 미확인 (63개 후보 조합 모두 미스매치). v1에서는 **keychain entry → CONFIG_DIR 매핑을 시도하지 않는 fallback** 적용.

대신 **로그인 상태는 `.claude.json`의 `oauthAccount.emailAddress` 유무로 판별**:
- 값 있음 → `✓ 로그인됨` + 이메일 표시
- 값 없음 또는 필드 없음 → `—`

근거: `.claude.json`은 CC가 직접 관리해 stale 가능성 낮음. keychain entry는 `/logout` 후에도 잠시 남을 수 있어 신뢰성 떨어짐. 매트릭스에서 keychain row는 "별도 (자동, alias 매핑 안 함)" 라벨로 단순 표기.

v1의 keychain 관련 작업은 모두 **수동 명령 출력**: `security delete-generic-password -s "Claude Code-credentials-<hash>"` 같은 명령을 사용자가 직접 실행하도록 안내. 자동 삭제 안 함.

**v2 후속 과제**: hash 알고리즘 reverse engineer 또는 CC 측 공식 API 활용해 entry ↔ CONFIG_DIR 매핑 자동화.
...
## 12a. Default 계정 보호 정책

`~/.claude` (명령어 `claude`) 는 사용자가 평소 가장 많이 쓰는 기본 계정이다. 실수로 mutation 작업이 적용되면 복구가 어렵다.

### 정책 요약

| 액션 | default `~/.claude` 처리 |
|---|---|
| **login** | 기본 선택지에서 **제외**. "기본 계정(claude) 로그인" 옵션 별도 추가 + 2차 확인 |
| **logout** | 기본 선택지에서 **제외**. "기본 계정(claude) 로그아웃" 옵션 별도 추가 + 2차 확인 |
| **edit** | 선택지에서 **완전 제외** (2차 확인 없음, 노출 자체 금지) |
| **unlink** | 선택지에서 **완전 제외** (기존 정책 유지) |
| **add** | default와 무관 (신규 alias 생성). 충돌 전환 분기에서도 default로 가지 않음 |
| **list** | 조회 전용이므로 포함 (읽기 전용, 변경 없음) |

### 근거

- edit·unlink는 디스크 구조·OAuth·zshrc를 바꾸는 파괴적 작업. 기본 계정이 영향을 받으면 일반 `claude` 사용이 즉시 중단됨.
- login·logout은 정당한 대상일 수 있으나 (기본 계정 재인증·세션 초기화), 의도치 않은 조작 방지를 위해 2차 확인 필수.
- `discover.sh list-dirs`는 default `~/.claude`를 포함해 반환할 수 있으므로, 각 SKILL.md가 직접 `if [ "$d" = "$HOME/.claude" ]` 필터를 적용한다.
...
### v0.4 구현 완료 (2026-05-26 — unlink 자동화)

- **계정 연동 해제(unlink) 자동화** — `auth_logout` + `remove_block` + `archive_dir` 3개 op를 operation plan에 추가. shell-mode=auto 시 전체 자동, manual 시 명령 출력만. v0.3의 `claude auth logout` 지원 덕분에 keychain 자동 정리 가능.

### v2 후속 (대형)
- **외부 alias 자동 unlink** — 손으로 만든 alias의 안전한 파싱·제거 방법론 정립 후
...
| 버전 | 내용 |
|---|---|
| v0.1.0 | plugin 골격 + 슬래시 명령 발견 검증 |
| v0.1.1~0.2.0 | discover/shell-rc/safety/plan-*/matrix 헬퍼 + 4-skill refactor (add/list/edit/unlink) |
| v0.3.0 | login/logout sub-skill (`claude auth login/logout/status` 활용), matrix.sh의 status cross-check |
| v0.4.0~0.4.3 | unlink 완전 자동화 (auth_logout + remove_block + archive_dir), 객관식 패턴 명시, 셸 모드 객관식 제거 |
| v0.4.4 | **Codex 리뷰 P0 + P1-1·3·5 반영** — archive 무결성 검증·remove 분리·logout 결과 보고·remove_block alias 검증·symlink 백업 롤백·default 계정 보호 통일 (§12a) |

현재 동작: macOS + zsh에서 add/list/edit/login/logout/unlink 6개 sub-skill + 메뉴 dispatcher. 단위 테스트 131/131. 사용자 환경에서 MV-1·4·unlink 실검증 완료.

### 미반영 Codex 리뷰 (이전 종합 리뷰, v0.5+ 후보)

- **P1-2** 활성 세션 게이트 false negative/positive — `pgrep -f claude` + env 문자열 완전 일치. realpath 정규화 + default config(env 부재) 별도 처리 필요
