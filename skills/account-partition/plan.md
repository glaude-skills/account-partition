...
### Task B.2: keychain-discover.sh — **v1에서 제외** (v2 후속)

Phase A.2에서 hash 알고리즘 reverse engineer 실패 → fallback 정책 적용. v1에서는 keychain entry → CONFIG_DIR 매핑 시도 안 함. 로그인 상태는 `.claude.json`의 `oauthAccount.emailAddress`로 판별. 따라서 keychain-discover.sh는 v1에서 작성하지 않음.

원래 Task B.2 내용(아래)은 v2 참고용:
